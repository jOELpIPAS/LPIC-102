...
if [[ $p4 = "ss -4t src 192.168/16" || $p4 = 'ss -t4 src 192.168/16' ]]; then 
    echo '   ✅'; echo
else
...
# Question 5
read -p $'\e[32m5) Show all TCP established sockets connected to the remote network 192.168.100.22:\e[33m\n   ' p5
if [[ $p5 = "ss -t state established dst 192.168.100.22" ]]; then 
    echo '   ✅'; echo
else
    echo -e '   \e[31m❌ CORRECT ANSWER >>\e[0m ss -t state established dst 192.168.100.22'
    echo
fi


# Question 6
read -p $'\e[32m6) Show all TCP sockets connected to the remote network 192.168.100.22 on port http:\e[33m\n   ' p6
if [[ $p6 = 'ss -t dst 192.168.100.22 dport = :http' || $p6 = 'ss -t dport = :http dst 192.168.100.22' || $p6 = 'ss -t dst 192.168.100.22 dport = :80' || $p6 = 'ss -t dport = :80 dst 192.168.100.22' || $p6 = 'ss -t dst 192.168.100.22 dport = http' || $p6 = 'ss -t dst 192.168.100.22 dport = 80' || $p6 = 'ss -t dport = http dst 192.168.100.22' || $p6 = 'ss -t dport = 80 dst 192.168.100.22' || $p6 = 'ss -t dst 192.168.100.22 dport 80' || $p6 = 'ss -t dport 80 dst 192.168.100.22' || $p6 = 'ss -t dst 192.168.100.22 dport = :80' || $p6 = 'ss -t dport = :80 dst 192.168.100.22' ]]; then 
    echo '   ✅'; echo
else
    echo -e '   \e[31m❌ CORRECT ANSWER >>\e[0m ss -t dst 192.168.100.22 dport = :http || ss -t dst 192.168.100.22 dport = :80'
    echo
fi
