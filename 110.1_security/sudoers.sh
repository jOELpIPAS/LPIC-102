...
# Question 6
read -p $'\e[32m6) Enter a specification that enables qlsaico to run systemctl status apache2 as user pipas, and restricting hosts to 192.168.100.22:\e[33m\n   ' p6
if [[ $p6 = 'qlsaico 192.168.100.22=(pipas) /usr/bin/systemctl status apache2' ]]; then 
    echo '   ✅'; echo
else
    echo -e '   \e[31m❌ CORRECT ANSWER >>\e[0m qlsaico 192.168.100.22=(pipas) /usr/bin/systemctl status apache2'
    echo
fi
