...
# Question 3
read -p $'\e[32m3) Enter 3 commands used for listing the network interfaces, separated by "|":\e[33m ' p3
...
if [[ $p5 = "ifdown -a" ]]; then 
    ...
    echo -e '   \e[31m❌ CORRECT ANSWER >>\e[0m ifdown -a'
    echo
fi


# Question 6
read -p $'\e[32m6) File used by ifup and ifdown that contain the interface definitions:\e[33m ' p6
...
# Question 8
read -p $'\e[32m8) Enter the command that enables all the interfaces definded with "auto" in /etc/network/interfaces:\e[33m ' p8
if [[ $p8 = 'ifup -a' ]]; then 
    echo '   ✅'; echo
else
    echo -e '   \e[31m❌ CORRECT ANSWER >>\e[0m ifup -a'
    echo
fi
...
# Question 13
read -p $'\e[32m13) Complete the following entry that setup eth0 to dhcp settings in /etc/network/interfaces "i _ _ _e  eth0  i _ e _   _ _ _ _":\e[33m ' p13
